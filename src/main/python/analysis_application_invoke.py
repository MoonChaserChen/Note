...
apps = {"voucher-service": "inaop@b245f0483fb7500", "voucher-task-service": "inaop@b0aad5e0aead514"}
...
class QueryData:
    def __init__(self, start_time, end_time, pid):
        ...
        self.filters = QueryFilters(pid).__dict__
    ...
    def __str__(self):
        return "query=" + json.dumps(self.__dict__)
...
for app in apps:
    ...
    with connection:
        with connection.cursor() as cursor:
            ...
            cursor.executemany(sql, list(map(lambda e: (app, e['rpc'], query_data.get_date(), int(e['rt']), int(e['count'])), j["data"]["data"])))
        connection.commit()
